...
describe('enforceContextLimitForRoute', () => {
  ...
  test('throws ContextLengthExceededError when context exceeds alias metadata context_length', () => {
    // Tiny window: OVER_WINDOW_SIZE tokens via alias metadata.
    ...
    const config = aliasConfig({
      enforce_limits: true,
      metadata: { overrides: { context_length: OVER_WINDOW_SIZE } } as any,
    });
  ...
  test('does not throw when context is under the window', () => {
    ...
    const config = aliasConfig({
      enforce_limits: true,
      metadata: { overrides: { context_length: contextWindowSize } } as any,
    });
