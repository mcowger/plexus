...
import { getConfigPath, validateConfig, loadConfig, getConfig, QuotaDefinition } from '../../config';
...
export async function registerUserQuotaRoutes(fastify: FastifyInstance) {
     ...
     */
    fastify.get('/v0/management/user-quotas', async (_request: FastifyRequest, reply: FastifyReply) => {
        try {
     ...
     */
    fastify.get('/v0/management/user-quotas/:name', async (request: FastifyRequest, reply: FastifyReply) => {
        try {
     ...
     */
    fastify.post('/v0/management/user-quotas/:name', async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        try {
            ...
            if (!/^[a-z0-9][a-z0-9-_]{1,62}$/.test(name)) {
                return reply.code(400).send({
                    error: {
                        message: 'Invalid quota name. Must be a slug (lowercase letters, numbers, hyphens, underscores, 2-63 characters)',
     ...
     */
    fastify.patch('/v0/management/user-quotas/:name', async (request: FastifyRequest, reply: FastifyReply) => {
        const configPath = getConfigPath();
        if (!configPath) {
            return reply.code(500).send({ error: 'Configuration path not determined' });
        }
     ...
     */
    fastify.delete('/v0/management/user-quotas/:name', async (request: FastifyRequest, reply: FastifyReply) => {
        const configPath = getConfigPath();
        if (!configPath) {
            return reply.code(500).send({ error: 'Configuration path not determined' });
        }
