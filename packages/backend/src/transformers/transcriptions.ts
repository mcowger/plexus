...
export class TranscriptionsTransformer {
   ...
   */
  async transformResponse(response: any, format: string): Promise<UnifiedTranscriptionResponse> {
    if (format === 'text') {
   ...
   */
  async formatResponse(response: UnifiedTranscriptionResponse, format: string): Promise<any> {
    if (format === 'text') {
