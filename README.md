...
## ⚠️ Breaking Change: Configuration Migration to Database (Coming Soon)

**All configuration is moving from YAML files to the database.** This change will be released soon and affects all users upgrading from previous versions.
...
1. `adminKey` deprecated in YAML - use `ADMIN_KEY` env var (fallback available with warning)
2. Configuration file watching removed - changes to `plexus.yaml` after import are ignored
3. Management API returns JSON instead of YAML
4. Config editor page removed - use the Providers, Models, Keys, and Settings UI pages

### After Migration

Once migrated successfully, you may remove `plexus.yaml` and `auth.json` (keep backups). All future configuration changes should be made through the web UI or management API. The `ADMIN_KEY` environment variable should be set for clean startup without warnings.
