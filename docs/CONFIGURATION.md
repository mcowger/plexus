...
| `strip_unsigned_thinking` | Removes `thinking` blocks that carry no `signature` before dispatch. Anthropic requires a signature on every thinking block and rejects one without it with a hard HTTP 400 (`messages.N.content.0.thinking.signature: Field required`); signatures are lost when history arrives on a wire format that cannot carry them, e.g. OpenAI chat-completions `reasoning_content` replayed from another model or an earlier Claude session. Enabled automatically under the same gate as `normalize_anthropic_tool_ids` (Anthropic Messages wire format **and** an Anthropic-looking provider). Other Messages-speaking providers such as Kimi require unsigned thinking to remain on historical tool-call turns, so it is **not** applied to them — force it on for a strict Anthropic-compatible gateway on another domain with `{ "name": "strip_unsigned_thinking", "options": {}, "enabled": true }`, or set `enabled: false` to opt out. The reactive strip-and-retry also matches this error as a fallback for unknown gateways. |
